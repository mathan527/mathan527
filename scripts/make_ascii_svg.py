...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">mathan kumar</text>')
...
# "mathan@github:~$ whoami Mathan Kumar P" — cursor offset after the name
WHOAMI_TEXT = "mathan@github:~$ whoami "
NAME_TEXT = "Mathan Kumar"
# approx pixel offset: font-size 13px monospace, each char ~7.8px
CURSOR_X = PAD + int((len(WHOAMI_TEXT) + len(NAME_TEXT)) * 7.8)
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'mathan@github:~$ whoami <tspan fill="{INK}">Mathan Kumar</tspan></text>')
